...
function _parse_snapshots!(model::JuMP.Model)
    ...
    # Set up `T`.
    _iesopt(model).model.T = _ID.(1:(config.count))
    ...
    if !isnothing(config.names)
        ...
        names = ["t$t" for t in _iesopt(model).model.T]
    end
    ...
    if !isnothing(config.weights)
        ...
        elseif config.weights isa Number
            weights = ones(length(_iesopt(model).model.T)) .* config.weights
        end
    else
        weights = ones(length(_iesopt(model).model.T))
    end
    ...
    if !isnothing(config.representatives)
        ...
        repr_indices = [ismissing(_repr[t]) ? missing : findfirst(==(_repr[t]), names) for t in _iesopt(model).model.T]
        if any(isnothing.(repr_indices))
            @critical "Missing repesentative detected; make sure that all names are actual Snapshot names"
        end

        representatives =
            [ismissing(_repr[t]) ? t : _iesopt(model).model.T[repr_indices[t]] for t in _iesopt(model).model.T]

        @info "Activated representative Snapshots" n = sum(is_representative)
        _has_representative_snapshots(model) = true

        if any(weights[t] != weights[1] for t in _iesopt(model).model.T[2:end])
            @critical "Representative Snapshots require equal `weights` for every Snapshot"
        end
    else
        is_representative = ones(Bool, length(_iesopt(model).model.T))
        representatives = _iesopt(model).model.T
    end
    ...
    @info "Parsed $(length(_iesopt(model).model.snapshots)) snapshots"
...
function _snapshot_aggregation!(model::JuMP.Model, data::Vector)
    ...
    if !isnothing(config.aggregate)
        ...
        scale = config.aggregate
        return [sum(@view data[((t - 1) * scale + 1):(t * scale)]) / scale for t in _iesopt(model).model.T]
    else
        return data
    end
end
